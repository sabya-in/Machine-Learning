...
batch_size = 800
exploration = int(input("Enter Batch Size for neural network training after each game int(default 800):") or batch_size)
direction = 'none'
q = dqn(state_set_size,action_set_size,exploration)
for episodes in range(10000):
    state = env.reset()
    state = np.reshape(state, [1, state_set_size])
    for time in range(300):
        env.render()
        action = q.action_decide(state,exploration,math.sqrt(episodes+1))
